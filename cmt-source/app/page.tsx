...
export default function Home() {
  ...
  const runSearch = (forced?: string) => {
    const nextQuery = forced ?? query;
    if (forced) setQuery(forced);
    const q = normalize(nextQuery);
    const tokens = q.split(" ").filter((token) => token.length > 1 && !["co", "nao", "cho", "o", "gan", "day", "duoc", "khong", "gi", "mot"].includes(token));
    const scored = entities.map((entity) => {
      const related = signals.filter((signal) => signal.entity_id === entity.entity_id);
      const haystack = normalize([entity.entity_name, entity.address, entity.location, ...entity.categories_seen, ...entity.items_seen, ...entity.good_points, ...related.map((signal) => signal.raw_text)].join(" "));
      const score = tokens.reduce((sum, token) => sum + (haystack.includes(token) ? 1 : 0), 0);
      return { id: entity.entity_id, score };
    }).filter((item) => item.score > 0).sort((a, b) => b.score - a.score);
    setResultIds(scored.length ? scored.map((item) => item.id) : entities.map((item) => item.entity_id));
    setSearchRan(true); go("search");
  };
    ...
    <main className="app-shell">
      ...
      {view === "share" && (
        <div className="screen inner-screen">
          ...
          {shareStep === 2 && (
            <section className="share-body">
              {extractReady && sourceType === "PHOTO" && <div className="confidence"><Icon name="check" /><div><strong>Đã nhận diện ảnh</strong><small>Độ tin cậy mô phỏng 93%</small></div><span>AI EXTRACT</span></div>}
              ...
              <div className="data-rule"><Icon name="pin" /><p><strong>Tên + địa chỉ chỉ trở thành dữ liệu tin cậy sau bước này.</strong><br />Category không được dùng để quyết định hai nơi có phải cùng một ENTITY.</p></div>
              <button className="main-action" disabled={!entityName.trim() || !address.trim()} onClick={() => setShareStep(3)}><Icon name="check" /> Xác nhận đúng địa điểm <b>→</b></button>
            </section>
          )}
          ...
          {shareStep === 3 && (
            <section className="share-body">
              ...
              <label className="feeling-box"><span>ĐIỀU GÌ Ở ĐÂY LÀM BẠN THẤY TỐT?</span><textarea autoFocus rows={6} value={rawText} onChange={(event) => setRawText(event.target.value)} placeholder="Ví dụ: Makeup tự nhiên, chụp ảnh lên rất đẹp…" /><small>{rawText.length}/300 · Chỉ cần vài dòng thật</small></label>
              {rawText.trim() && <div className="understood"><div className="understood-head"><Icon name="spark" /><strong>AI hiểu từ nội dung của bạn</strong><span>MÔ PHỎNG</span></div><dl><div><dt>Category</dt><dd>{derived.category}</dd></div><div><dt>Món / dịch vụ</dt><dd>{derived.item}</dd></div><div><dt>Điểm tốt</dt><dd>{derived.points.map((point) => <span key={point}>{point}</span>)}</dd></div></dl><p>Nội dung gốc luôn được giữ nguyên.</p></div>}
              <button className="main-action" disabled={rawText.trim().length < 8} onClick={saveExperience}>Chia sẻ trải nghiệm tốt <b>→</b></button>
            </section>
          )}

          {shareStep === 4 && (
            <section className="success-body"><div className="success-mark"><span>✓</span><i /><i /></div><p className="kicker">GÓP 1 <span>→</span> NHẬN 10</p><h2>Cảm ơn bạn đã góp<br />một trải nghiệm thật.</h2><p>Điều tốt này đã được lưu thành một SIGNAL và gắn với đúng ENTITY đã xác nhận.</p><div className="reward-card"><span>+10</span><div><strong>Lượt khám phá</strong><small>Tìm thêm điều tốt từ cộng đồng</small></div></div><button className="main-action" onClick={() => openEntity(selectedEntityId)}>Xem nơi vừa đóng góp <b>→</b></button><button className="secondary-action" onClick={() => { resetShare(); go("home"); }}>Về trang chủ</button><small className="saved-id">Đã lưu cục bộ · {savedSignalId.slice(-8)}</small></section>
          )}
        </div>
      )}
      ...
      {view === "search" && (
        <div className="screen inner-screen search-screen">
          ...
          {!searchRan ? <section className="search-suggestions"><p>THỬ HỎI TIKTAK</p>{["Ở Hải Phòng có quán phở nào ngon?", "Quanh Cầu Giấy có cafe nào yên tĩnh?", "Có chỗ sửa điều hòa được mọi người khen không?", "Gần đây có chỗ makeup đẹp không?"].map((item) => <button key={item} onClick={() => runSearch(item)}><span>“{item}”</span> →</button>)}</section> : <section className="results"><div className="result-summary"><Icon name="spark" /><div><small>TIKTAK HIỂU BẠN ĐANG TÌM</small><strong>{query}</strong></div></div><div className="result-title"><p>{resultIds.length} nơi có trải nghiệm phù hợp</p><span>Không quảng cáo · Không xếp hạng</span></div>{resultIds.map((id) => { const entity = entities.find((item) => item.entity_id === id)!; const related = signals.filter((item) => item.entity_id === id); return <button className="result-card" key={id} onClick={() => openEntity(id)}><div className="result-visual"><span>{entity.categories_seen[0]?.slice(0,1)}</span><small>DỮ LIỆU<br />MINH HỌA</small></div><div className="result-copy"><div className="result-meta"><span><Icon name="pin" /> {entity.location}</span><b>{related.length} trải nghiệm</b></div><h3>{entity.entity_name}</h3><p>{entity.address}</p><div className="praise">Mọi người khen: {entity.good_points.slice(0,3).map((point) => <span key={point}>{point}</span>)}</div><blockquote>“{related[0]?.raw_text}”</blockquote></div><i>→</i></button>; })}</section>}
        </div>
      )}

      {view === "entity" && selectedEntity && (
        <div className="screen inner-screen entity-screen">
          <div className="entity-cover"><button onClick={() => go(searchRan ? "search" : "home")} aria-label="Quay lại"><Icon name="back" /></button><span>DỮ LIỆU MINH HỌA</span><div className="entity-letter">{selectedEntity.entity_name.slice(0,1)}</div></div>
          <section className="entity-content"><p className="entity-type">ENTITY · {selectedEntity.categories_seen.join(" · ")}</p><h1>{selectedEntity.entity_name}</h1><p className="entity-address"><Icon name="pin" /> {selectedEntity.address}</p><div className="entity-actions"><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(selectedEntity.entity_name + " " + selectedEntity.address)}`} target="_blank" rel="noreferrer"><Icon name="route" /> Chỉ đường Google Maps</a></div><div className="evidence-strip"><div><strong>{selectedSignals.length}</strong><span>trải nghiệm thật</span></div><div><strong>{selectedEntity.items_seen.length}</strong><span>món / dịch vụ</span></div><div><strong>{selectedEntity.good_points.length}</strong><span>điểm tốt</span></div></div><section className="good-at"><small>MỌI NGƯỜI ĐÃ KHEN</small><div>{selectedEntity.good_points.map((point) => <span key={point}><Icon name="check" /> {point}</span>)}</div></section><section className="signals-list"><div className="list-heading"><small>CÁC SIGNAL LIÊN QUAN</small><span>{selectedSignals.length} đóng góp</span></div>{selectedSignals.map((signal) => <article key={signal.signal_id}><div className="signal-top"><span>{signal.contributor === "Bạn" ? "B" : "C"}</span><div><strong>{signal.contributor}</strong><small>{new Date(signal.created_at).toLocaleDateString("vi-VN")} · {signal.source_type === "PHOTO" ? "Có ảnh xác nhận" : "Nhập tay"}</small></div></div><blockquote>“{signal.raw_text}”</blockquote><div className="signal-tags"><span>{signal.category}</span><span>{signal.item_mentioned}</span></div></article>)}</section><div className="entity-rule"><strong>Một nơi, nhiều trải nghiệm</strong><p>Các SIGNAL này cùng ENTITY vì tên + địa chỉ đã xác nhận trùng nhau — không phải vì chúng cùng category.</p></div></section>
        </div>
      )}

      {view === "turns" && (
        <div className="screen inner-screen turns-screen"><div className="screen-heading compact"><button onClick={() => go("home")} aria-label="Quay lại"><Icon name="back" /></button><div><small>GÓP 1 · NHẬN 10</small><h1>Lượt của bạn</h1></div></div><section className="turn-balance"><span><Icon name="turns" /></span><small>LƯỢT KHÁM PHÁ HIỆN CÓ</small><strong>{turns}</strong><p>Mỗi trải nghiệm thật bạn góp giúp cộng đồng có thêm điều tốt để tìm lại.</p></section><div className="turn-stats"><div><strong>{contributionCount}</strong><span>Đã góp</span></div><div><strong>{contributionCount * 10}</strong><span>Lượt nhận thêm</span></div></div><section className="my-contributions"><div className="list-heading"><small>ĐÓNG GÓP CỦA BẠN</small><span>{contributionCount}</span></div>{contributionCount === 0 ? <div className="empty-contribution"><span><Icon name="share" /></span><h3>Bạn chưa góp trải nghiệm nào</h3><p>Bắt đầu bằng một hóa đơn hoặc biển hiệu của nơi bạn thấy tốt.</p><button className="main-action" onClick={startShare}>Chia sẻ trải nghiệm tốt</button></div> : userSignalIds.map((id) => { const signal = signals.find((item) => item.signal_id === id)!; const entity = entities.find((item) => item.entity_id === signal?.entity_id); return <button key={id} onClick={() => entity && openEntity(entity.entity_id)}><span><Icon name="check" /></span><div><strong>{entity?.entity_name}</strong><p>{signal.raw_text}</p></div><i>→</i></button>; })}</section></div>
      )}
